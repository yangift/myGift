...
<script type="text/javascript">
	function page(pageNum, pageCount) {
		...
		document.form1.action = "order?flag=query&cusId=<%=cus.getCusId()%>&page=" + pageNum;
		...
		<section id="main-content">
			<section class="wrapper">
				...
				<div class="row">
					<div class="col-lg-12">
						...
						<ol class="breadcrumb">
							...
							<li><i class="fa fa-files-o"></i>历史订单</li>
						</ol>
					</div>
				</div>
				<div class="col-md-12">
				...
				<div class="col-md-5 col-md-offset-8 text-center ">
					...
					<a href = "cst_orderAdd.jsp?cusId=<%=cus.getCusId()%>"><button class="btn btn-info" type = "button" >新建</button></a>
				</div>
				</div>
							...
							<table class="table">
								...
								<tbody >
									...
									SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
										List<Orders> list = (List<Orders>) request
									...
									<tr>
										...
										<td><%=sdf.format(order.getOrdDate())%></td>
										<td><%=order.getOrdAddress()%></td>
										<td><%=order.getOrdState()%></td>
										<td><a
											href="order?flag=initEdit&assId=<%=order.getOrdId()%>&cusId=<%=cus.getCusId()%>"
											title="编辑交往记录"> <span class="glyphicon glyphicon-pencil">&nbsp;</span>
										</a> <a href="order?flag=del&assId=<%=order.getOrdId()%>&cusId=<%=cus.getCusId()%>" title="删除"> <span
